...
def init(object_directory=None, globs={}):
    ...
    if object_directory:
        sage.structure.sobj.base=object_directory
...
def save_session(filename):
    ...
    for k in v:
        x = sage_globals[k]
        try:
            _ = sage.structure.sobj.loads(sage.structure.sobj.dumps(x))
    ...
    sage.structure.sobj.save(D, filename)
